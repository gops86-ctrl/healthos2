...
fun HealthOSRoot() {
    ...
    LaunchedEffect(repository) {
        ...
        merge(
            ...
            val result = HealthApiClient(
                BuildConfig.HEALTHOS_API_BASE_URL,
                BuildConfig.HEALTHOS_API_KEY
            ).uploadSnapshot(
                // MCP history depends on the canonical snapshot containing every
                // Room metric record, not only the latest value for each metric type.
            ...
            result.onFailure { error ->
                android.util.Log.w("HealthOS", "Canonical snapshot upload failed", error)
            }
        }
    }
    ...
    val syncGarminActivities: suspend (String, (Int, String) -> Unit) -> Int = { range, onProgress ->
        ...
        candidates.forEachIndexed { index, activity ->
            ...
            val stravaCandidates = database.activityDao().findBySourceBetween(
                source = "STRAVA",
            ...
            if (duplicate) {
                existingGarmin?.let { database.activityDao().deleteById(it.id) }
            } else {
                database.activityDao().insertAll(listOf(activity))
                imported++
            }
            onProgress(
                70 + ((index + 1) * 30 / candidates.size.coerceAtLeast(1)),
                "Checking activities ${index + 1} / ${candidates.size}…"
            )
        }
        imported
    }
    ...
    val syncGarmin: suspend (String, (Int, String) -> Unit) -> Int = { range, onProgress ->
        ...
        while (!chunkStart.isAfter(end)) {
            ...
            onProgress(
                ((completedDays.toDouble() / totalDays) * 100).toInt(),
                "Syncing ${chunkStart} → ${chunkEnd}…"
            )
            ...
            onProgress(
                ((completedDays.toDouble() / totalDays) * 70).toInt().coerceAtMost(70),
                "Synced ${completedDays.coerceAtMost(totalDays)} / $totalDays days"
            )
            chunkStart = chunkEnd.plusDays(1)
        }

        if (range == "7D" || range == "30D") {
            syncGarminActivities(range) { progress, stage ->
                onProgress(70 + (progress * 30 / 100), stage)
            }
        } else {
            onProgress(100, "Garmin sync complete")
        }

        val snapshotResult = HealthApiClient(
            BuildConfig.HEALTHOS_API_BASE_URL,
            BuildConfig.HEALTHOS_API_KEY
        ).uploadSnapshot(
            // Refresh the canonical API after sync so MCP consumers see newly
            // imported Garmin metrics and activities without restarting the app.
            metrics = database.healthMetricDao().observeAllHistory().first().map { it.toDomain() },
            activities = repository.observeActivities().first(),
            workouts = repository.observeWorkouts().first(),
            nutrition = repository.observeNutritionEntries().first(),
        ...
        snapshotResult.onFailure { error ->
            android.util.Log.w("HealthOS", "Post-sync canonical snapshot upload failed", error)
        }
    ...
    HealthOSNavHost(
        ...
        onGarminSync = syncGarmin,
        onDeleteNutritionDay = { date ->
...
private fun HealthMetricEntity.toDomain(): HealthMetric = HealthMetric(
...
private fun isLikelySameActivity(strava: ActivityEntity, garmin: ActivityEntity): Boolean {
    ...
    val elapsedDifference = if (strava.elapsedDurationSeconds != null && garmin.elapsedDurationSeconds != null) {
        abs(strava.elapsedDurationSeconds - garmin.elapsedDurationSeconds).toDouble()
    } else null
    val durationMatch = durationDifference <= max(5 * 60.0, maxDuration * 0.08) ||
        (elapsedDifference != null && elapsedDifference <= max(5 * 60.0, max(strava.elapsedDurationSeconds ?: 0L, garmin.elapsedDurationSeconds ?: 0L) * 0.08))
    if (!durationMatch) return false

    val distanceDifference = if (strava.distanceMeters != null && garmin.distanceMeters != null && strava.distanceMeters > 100.0 && garmin.distanceMeters > 100.0) {
        abs(strava.distanceMeters - garmin.distanceMeters) / max(strava.distanceMeters, garmin.distanceMeters)
    } else null
...
private fun activityCategory(activity: ActivityEntity): String {
    val type = activity.activityType.uppercase()
    val name = activity.name.orEmpty().lowercase()
    return when {

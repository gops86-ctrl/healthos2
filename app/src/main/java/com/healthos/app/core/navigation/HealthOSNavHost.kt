...
@Composable
fun HealthOSNavHost(repository: HealthRepository, onGarminSync: suspend (String, (Int, String) -> Unit) -> Int, onDeleteNutritionDay: suspend (Long) -> Unit) {
        ...
        NavHost(nav, "overview", modifier = Modifier.padding(padding)) {
            composable("overview") {
                CompositionLocalProvider(
                    LocalWhatChangedClick provides { nav.navigate("what_changed") },
                    LocalProfileClick provides { nav.navigate("profile") },
                    LocalUserProfile provides profile
                ) {
                    OverviewScreen(
                        onMetricClick = { nav.navigate("metric/${it.type.name}") },
                        onWeightClick = { nav.navigate("weight") },
                        onNutritionClick = { nav.navigate("nutrition") },
                        onActivityClick = { nav.navigate("activity/${it.id}") },
                        onWorkoutClick = { nav.navigate("workout/${it.id}") },
            ...
            composable("data") { val vm = viewModel<com.healthos.app.feature.data.DataViewModel>(factory = factory); DataScreen(vm, onGarminSync) }

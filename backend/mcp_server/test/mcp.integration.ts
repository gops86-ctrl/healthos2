...
function startMockHealthApi(): Promise<ReturnType<typeof createServer>> {
  const server = createServer((req, res) => {
    ...
    const start = Number(
      url.searchParams.get('start_millis') ?? Number.NEGATIVE_INFINITY
    );

    const end = Number(
      url.searchParams.get('end_millis') ?? Number.POSITIVE_INFINITY
    );

    const range = <T extends { recordedAtMillis: number }>(
      records: T[]
    ) =>
      records.filter(
        record =>
          record.recordedAtMillis >= start &&
          record.recordedAtMillis <= end
      );
    ...
    switch (url.pathname) {
      case '/health/metrics': {
        ...
        body = range(expectedMetrics).filter(
          metric => !metricType || metric.metricType === metricType
        );
      ...
      case '/health/activities':
        body = range(expectedActivities);
        break;

      case '/health/workouts':
        body = range(expectedWorkouts);
        break;

      case '/health/nutrition':
        body = range(expectedNutrition);
        break;

      case '/health/body':
        body = range(expectedBody);
        break;

      case '/health/labs':
        body = range(expectedLabs);
        break;
...
async function waitForMcpServer(): Promise<void> {
  ...
  throw new Error(
    'MCP server did not become healthy within 5 seconds'
  );
}
      ...
      new Promise<T>((_, reject) => {
        timer = setTimeout(
          () =>
            reject(
              new Error(
                `${label} timed out after ${TEST_TIMEOUT_MS}ms`
              )
            ),
          TEST_TIMEOUT_MS
        );
...
const mockApiUrl =
  `http://127.0.0.1:${serverPort(mockApi)}`;
...
try {
  ...
  client = new Client({
    name: 'healthos-integration-test',
    version: '1.0.0'
  });
  ...
  if (
    JSON.stringify(actualToolNames) !==
    JSON.stringify(expectedSorted)
  ) {
    throw new Error(
      `Unexpected tool list: ${JSON.stringify(actualToolNames)}`
    );
  }

  const metricHistory = await callJson(
    client,
    'get_metric_history',
    {
  ...
  if (
    JSON.stringify(metricHistory) !==
    JSON.stringify(expectedMetrics.slice(1))
  ) {
    throw new Error(
      `Unexpected metric history: ${JSON.stringify(metricHistory)}`
    );
  }

  const currentMetrics = await callJson(
    client,
    'get_current_metrics',
    {
      metricType: 'RESTING_HR'
  ...
  if (
    ...
    JSON.stringify(expectedCurrentMetrics)
  ) {
    throw new Error(
      `Unexpected current metrics: ${JSON.stringify(currentMetrics)}`
    );
  }
  ...
  for (const [toolName, expected] of checks) {
    ...
    if (
      ...
      throw new Error(
        `Unexpected ${toolName} result: ${JSON.stringify(actual)}`
      );
    }
  }

  console.log(
    `MCP integration test passed: ${expectedToolNames.length} tools verified end-to-end.`
  );
} finally {
  await withTimeout(
